...
<8> Port forwarding is configured using `portBinding` section. It contains a list of `exposedPort` and `port` separated by arrow (_->_). If only one port is provided, *Cube* will expose the same port number. In this example the container port 8089 is exposed to 8089 and container port 8080 is exposed to 8081.
