...
When _Cube_ is started it will check if the +serverUri+ contains the _boot2docker_ word, and if it is the case it will check if you have set system environment or java property called `DOCKER_HOST`. In case it is set then it will substitute _boot2docker_ with given value. If property is not set then it will run the command +boot2docker ip+ to get the ip and substitute the _boot2docker_ sentence to the ip returned by that command. 
>>>>>>> a0efe23e43ec051af73390bc9fb55ce72e5a6ce1
