...
These certificates are copied by _boot2docker_ by default at +~/.boot2docker/certs+. 
If this propery is not set and the +serverUri+ contains +boot2docker+, then this property is automatically configured to +~/.boot2docker/certs+ so you don't need to worry to set for each environment.
