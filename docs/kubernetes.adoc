...
1. If using `Minishift` to spin up a local Kubernetes cluster, login in as `admin` to create the default namespace or
...
`cube.auth.token` or by username and password using properties `cube.username` and `cube.password` respectively.
