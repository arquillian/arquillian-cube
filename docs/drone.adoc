...
It seems logical to integrate Arquillian Drone with Cube in order to utilize the powerful features of Drone, and let Cube configure Docker things for you.

This integration automatically performs the following operations:
...
dockerRegistry:: Use this to override the default docker registry with a user specified registry - Don't forget the trailing slash (i.e. _your.repo**/**_)
...
TIP: Even when using the `STATIC` naming strategy it may be useful to override the container name for the 'browser' container, eg. when using parallel builds.
You can provide an override name via the `SELENIUM_CONTAINER_BASE_NAME` environment variable or as a system property on the command line.
