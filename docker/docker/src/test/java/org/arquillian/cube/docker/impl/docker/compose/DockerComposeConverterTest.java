...
public class DockerComposeConverterTest {
    ...
    @Test
    public void shouldStartServicewithMentionedNetworkInDockerComposeV2() throws URISyntaxException {
        ...
        assertThat(convert.getContainers().size(), is(2));
        assertThat(convert.getContainers()).containsKeys("test-classes_webapp", "test-classes_webapp2");
        CubeContainer webapp = convert.getContainers().get("test-classes_webapp");
        Collection<Network> webappNetworks = webapp.getNetworks();

        assertThat(webapp.getNetworkMode(), is("test-classes_front-tier"));
        assertThat(webappNetworks.size(), is(1));
    }

    @Test
    public void shouldStartServicewithDefaultNetworkIfNetworkMissingUnderServicesInDockerComposeV2() throws URISyntaxException, IOException {
        URI simpleDockerCompose = DockerComposeConverterTest.class.getResource("/simple-docker-compose-networks-v2.yml").toURI();
        DockerComposeConverter dockerComposeConverter = DockerComposeConverter.create(Paths.get(simpleDockerCompose));
        ...
        assertThat(convert.getContainers()).containsKeys("test-classes_webapp", "test-classes_webapp2");
        CubeContainer webapp2 = convert.getContainers().get("test-classes_webapp2");
        Collection<Network> webappNetworks = webapp2.getNetworks();

        assertThat(webapp2.getNetworkMode(), is("test-classes_default"));
        assertThat(webappNetworks.size(), is(1));
